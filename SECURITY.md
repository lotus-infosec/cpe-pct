...
| Secret scanning        | gitleaks: pre-commit (staged), pre-push (branch), CI (full), and as a gate in `npm run release`, which is how the owner deploys. `npm run deploy` is the ungated command underneath it, because the Deploy to Cloudflare button pre-populates its build and deploy fields from `package.json` and Cloudflare's builder has no gitleaks binary. GitHub push protection enabled. |
