...
// Stage 0 uses only settings/jobs/health_pings; Stage 1 activates the rest.
...
export const notifications = t('notifications', {
  ...
  status: text('status', { enum: ['pending', 'sent', 'read'] }).notNull().default('pending'),
  createdAt: text('created_at').notNull(),
  sentAt: text('sent_at'),
});
...
export const certifications = t('certifications', {
  id: text('id').primaryKey(),
  bodyId: text('body_id').notNull().references(() => bodies.id),
  name: text('name').notNull(),
...
export const ruleVersions = t('rule_versions', {
...
export const certRequirements = t('cert_requirements', {
  id: text('id').primaryKey(),
...
export const creditingRules = t('crediting_rules', {
  id: text('id').primaryKey(),
  ruleVersionId: text('rule_version_id').notNull().references(() => ruleVersions.id),
  ...
  rounding: text('rounding', { enum: ['floor_quarter', 'floor_half', 'floor_whole', 'nearest_quarter', 'exact'] }).notNull(),
...
export const constraints = t('constraints', {
  ...
  type: text('type', { enum: [
    'cycle_total', 'annual_min', 'category_min', 'category_max', 'activity_type_max',
...
export const certRelations = t('cert_relations', {
  ...
  relation: text('relation', { enum: ['earning_renews', 'credits_flow_down', 'requires_current', 'earning_credits'] }).notNull(),
...
export const heldCertifications = t('held_certifications', {
  ...
  status: text('status', { enum: ['active', 'lapsed', 'retired', 'pursuing'] }).notNull().default('active'),
  notes: text('notes'),
});

export const cycles = t('cycles', {
  id: text('id').primaryKey(),
  heldCertId: text('held_cert_id').notNull().references(() => heldCertifications.id),
  sequence: integer('sequence').notNull(),
  startsOn: text('starts_on').notNull(),
  endsOn: text('ends_on').notNull(),
  ruleVersionId: text('rule_version_id').notNull().references(() => ruleVersions.id),
  status: text('status', { enum: ['open', 'renewed', 'lapsed'] }).notNull().default('open'),
...
export const activities = t('activities', {
  ...
  status: text('status', { enum: ['draft', 'logged'] }).notNull().default('draft'),
  createdAt: text('created_at').notNull(),
});

// The heart of the application.
export const creditApplications = t('credit_applications', {
  id: text('id').primaryKey(),
  ...
  categoryKey: text('category_key'),
  status: text('status', { enum: ['planned', 'claimed', 'submitted', 'accepted', 'rejected'] }).notNull(),
...
}, (x) => [uniqueIndex('ca_activity_cert').on(x.activityId, x.heldCertId), index('ca_cycle').on(x.cycleId)]);
...
export const evidence = t('evidence', {
  ...
  extractionStatus: text('extraction_status', { enum: ['pending', 'done', 'no_text', 'failed', 'manual'] }).notNull(),
  extractionMethod: text('extraction_method'),
  uploadedAt: text('uploaded_at').notNull(),
});

export const activityEvidence = t('activity_evidence', {
  activityId: text('activity_id').notNull().references(() => activities.id),
...
export const renewals = t('renewals', {
  id: text('id').primaryKey(),
  closedCycleId: text('closed_cycle_id').notNull().references(() => cycles.id),
